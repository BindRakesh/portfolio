...
export class OfficeBuilder {
    ...
    createPillar(x, z) { 
        // 1. Visual Pillar
        const pillar = new THREE.Mesh(new THREE.BoxGeometry(2, 10, 2), Mats.matConcrete); 
        pillar.position.set(x, 5, z); 
        this.scene.add(pillar); 
        this.addStaticBody(x, 5, z, 2, 10, 2, true); 
        ...
        // FIX 3: ADD PHYSICS FOR PLANT (Small cylinder obstacle)
        // Positioned at x-1.5, z (same as anchor). Size roughly 0.8 width.
        this.addStaticBody(x - 1.5, 0.5, z, 0.8, 1, 0.8);
    }
    ...
    placeWorkstation(x, z, isInteractable, isHero) {
        ...
        // FIX 1: WIDER DESK PHYSICS (4.4 width instead of 3.5)
        this.addStaticBody(x, 1, z, 4.4, 2, 2.0, true); 

        // FIX 2: CHAIR PHYSICS (Box at z+1.2)
        this.addStaticBody(x, 0.5, z + 1.2, 0.8, 1, 0.8);
    }
